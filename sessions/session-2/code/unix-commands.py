...
training_examples = [
    ...
    dspy.Example(
        request="show me what's in this folder",
        command="ls"
    ).with_inputs("request"),
    ...
    dspy.Example(
        request="count the number of lines in file.txt",
        command="wc -l file.txt"
    ).with_inputs("request"),
    
    dspy.Example(
        request="show the last 20 lines of the log file",
    ...
    dspy.Example(
        request="make a new directory called projects",
        command="mkdir projects"
    ).with_inputs("request"),
    
    dspy.Example(
        request="delete the file called temp.txt",
        command="rm temp.txt"
    ).with_inputs("request"),
    ...
    dspy.Example(
        request="see how much disk space is available",
        command="df -h"
    ).with_inputs("request"),
    ...
    dspy.Example(
        request="show me the current directory path",
        command="pwd"
    ).with_inputs("request"),
    
    dspy.Example(
        request="copy file.txt to backup.txt",
    ...
    dspy.Example(
        request="show the contents of config.json",
        command="cat config.json"
    ).with_inputs("request"),
    
    dspy.Example(
        request="find files larger than 100MB",
        command="find . -size +100M"
    ).with_inputs("request"),
    
    dspy.Example(
        request="check network connectivity to google.com",
    ...
    dspy.Example(
        request="see who is logged into the system",
        command="who"
    ).with_inputs("request"),
]
